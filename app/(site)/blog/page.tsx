...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: siteSettings.blogPageEyebrow,
    description: siteSettings.blogPageIntro,
  };
}
    ...
    <section className="site-page-x mx-auto w-full max-w-7xl space-y-8 pb-14 pt-24 lg:pb-16 lg:pt-24">
      <div className="space-y-2">
        <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-primary">
          {siteSettings.blogPageEyebrow}
        </p>
        <h1 className="max-w-3xl font-serif text-2xl tracking-tight text-foreground sm:text-3xl">
          {siteSettings.blogPageHeading}
        </h1>
        <p className="max-w-2xl text-sm leading-6 text-muted-foreground">{siteSettings.blogPageIntro}</p>
      </div>
